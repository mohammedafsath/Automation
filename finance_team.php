...
{
   $query=mysql_query("select * from cri_uo_financeteam");
   $head=$_POST['is_head'];
   $flag=0;
   ...
   if($flag==0)
   {
		$temp=0;
		$query=mysql_query("select * from cri_uo_financeteam");
		$sql=mysql_query("select count(is_head)from cri_uo_financeteam where status=1");
		while($row=mysql_fetch_assoc($query))
	    {
		if($row['is_head'] == '1');
		$temp=1;
		...
		if($head=='1')
		{
			if($temp==1)
			{
				$delete=mysql_query("update cri_uo_financeteam set is_head='0'");
				$sql=mysql_query("insert into cri_uo_financeteam(user_name,is_head,status) VALUES ('".$_POST['user_name']."','".$_POST['is_head']."','1')");
			}
			else
				$sql=mysql_query("insert into cri_uo_financeteam(user_name,is_head,status) VALUES ('".$_POST['user_name']."','".$_POST['is_head']."','1')");
		}
		...
		{
			$sql=mysql_query("insert into cri_uo_financeteam(user_name,is_head,status) VALUES ('".$_POST['user_name']."','0','1')");
		}
	}
	else
	{
	...
	<?php include"navigation.php"; ?>
          ...
          <ul class="sidebar-menu">
           ...
           <li class="treeview">
			...
			<li class="active treeview"><a href="#">&nbsp;&nbsp;&nbsp;<i class="fa fa-circle-o text-aqua"></i><span>User Options</span> <i class="fa fa-angle-left pull-right text-white"></i></a>
				...
				<li class="active"><a href="finance_team.php">&nbsp;&nbsp;&nbsp;<i class="fa fa-circle-o text-yellow"></i>Finance Team</a></li>
      ...
      <div class="content-wrapper">
       ...
       <section class="content-header" align="center">
          <h1
			<font color="#006600" family="ariel">FINANCE TEAM</font>
          </h1>
       </section>
     ...
     <section class="content">   
		<section class="content">
          <div class="row">
			...
			<?php
			$i=0;
			$sql="select * from cri_uo_financeteam where status=1 order by id asc";
			$result=mysql_query($sql);
			$sql2 = "SELECT id from cri_uo_financeteam where is_head='1' and status=1";
			...
			?>
				<tr>
					...
					<td align=center><a href="finance_team.php?did=<?php echo $row['id']; ?>">Delete</a></td>
				</tr>
			<?php } ?>			
